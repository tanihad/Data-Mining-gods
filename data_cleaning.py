...
def avg_danceability():
    ...
    # Rename columns for clarity and to avoid conflicts in the merge
    df1.rename(columns={'danceability': 'danceability_spotify'}, inplace=True)
    df2.rename(columns={'name': 'track_name', 'spotify_id': 'track_id', 'danceability': 'danceability_fantano'},
               inplace=True)

    # Merge the datasets based on track name and track ID
    common_songs = pd.merge(df1, df2, on=["track_name", "track_id"], how='inner')

    # Print to check the first few entries to verify correct merge and column names
    print(common_songs[['track_name', 'track_id', 'danceability_spotify', 'danceability_fantano']].head())

    # Save to CSV, including the new danceability columns for both Spotify and Fantano
    common_songs.to_csv('common_songs.csv', index=False)

    # If you need to work further with album names and grouping
    dfGrouping = pd.read_csv('common_songs.csv')
    grouped = dfGrouping.groupby('album_name')
